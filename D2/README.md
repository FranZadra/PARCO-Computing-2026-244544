...
## Deliverable 2: Distributed Sparse Matrix-Vector Multiplication (SPMV) with MPI

This repository contains the C source files, the scripts used to run the tests, the results saved in CSV format, and the plotted data generated by a Python script, which were used to analyze and describe a distributed-memory implementation of Sparse Matrix-Vector Multiplication (SpMV).
...
│   ├── communication.c         # Ghost values exchange and MPI local SpMV
...
│   ├── weak_scaling_hybrid.csv    # MPI+OpenMP weak scaling benchmark results saved in CSV format
│   ├── benchmark_mpi.err          # output error console for MPI benchmarking
│   ├── benchmark_mpi.out          # output console for MPIs benchmarking
...
The comparison focuses on execution time, speedup, strong scaling and weak scaling across different matrices and different MPI configurations
...
#PBS -l select=6:ncpus=24:mem=16gb
```
